...
function initialize(passport) {
  ...
  passport.use('local.signup', new LocalStrategy({
    ...
    async (req, email, password, done) => {
      ...
      try {
        ...
        const registration = 'INSERT INTO users (user_id, password, first_name, last_name, email, created_at) VALUES ($1, $2, $3, $4, $5, $6) RETURNING user_id';
  ...
  passport.use('local.auth', new LocalStrategy(     //new LocalStrategy class takes 2 parameters
    ...
    async (email, password, done) => {//this second parameter tells passport how to authenticate our user with the given email/password credentials
      ...
      const queryUser = await db.query(`SELECT * FROM users WHERE email = $1`, [email]);
